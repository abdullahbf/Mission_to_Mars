...
def hemispheres(browser): 
    ...
    for hemisphere in range(4): 
        ...
        html = browser.html # Parse the resulting html with soup 
        img = soup(html, "html.parser")
        
        title = img.find("h2", class_= "title").text
        full_resolution_img = img.find_all("a")[3]
        href = full_resolution_img["href"]
        
        url = f"https://marshemispheres.com/{href}"
        
        if url not in hemisphere_image_urls:
